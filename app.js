(() => {
  ...
  // CONFIG
  // =========================
  const API_URL = "https://script.googleusercontent.com/macros/echo?user_content_key=AehSKLifI0TdMm1lTizjVqZdOGNOauKrcKQ0VXMn0mfnqy61GIYfQJ6EO8Ool4yssNaZNypfPdUEnr0rI5uSMgxPwoJ1sP4faP6WzwTUkc9tYOdTkD8FQ87cscGljYX5R4sP2HTgobm2L0r9h2f5vjqMJKsOT8u_bPYKHAWXzLX-Ta4F5BUh3YRZ4AKjOa3PMSr2gV3h8aRWnLfcXYBszQ7XM2bzfWtTRySEEA3yxtFx-OvUMVlquE_ZbSnXKV6CdxVZNNjyqsy7QfFR2O1g4u7qCyKE6JZnNw&lib=MFuM-0_fRIQw0bD-l033NBRsoTN8THAby";
...
<div class="card">
  ...
  // 🔗 suppression serveur (Drive + Sheet)
...
if (t.photos && t.photos.length > 0) {
  ...
  // ✅ affichage seulement (PAS DE SYNC)
  renderGallery(t.photos);
  renderPhotoCarousel(t.photos);
...
if (undoBtn) {
  undoBtn.onclick = async () => {
    ...
    // 🔧 migration driveId AVANT sauvegarde
(t.photos || []).forEach(p => {
  if (!p.driveId && p.url) {
    const extracted = extractDriveIdFromUrl(p.url);
    if (extracted) {
...
async function loadTreesFromSheets() {
  try {
    ...
    console.log("📥 Données chargées depuis Google Sheets :", data.length);

    trees = data; // ✅ Sheets = vérité

  } catch (e) {
    console.warn("❌ Sheets inaccessible → aucun arbre chargé", e);
    trees = []; // ✅ on vide, on ne ressuscite rien
  }
}
